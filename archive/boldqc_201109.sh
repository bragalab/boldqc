...
# Usage:
# boldqc_201109.sh file skip_trs
# e.g. sh /projects/b1134/tools/boldqc/boldqc_201109.sh /projects/b1134/raw/bids/SeqDev/sub-fMRIPILOT1030/ses-fMRIPILOT1030/func/sub-fMRIPILOT1030_ses-fMRIPILOT1030_task-REST02_acq-CBS2p4_bold.nii.gz 4
...
mcflirt -in $TMPDIR/${filename}_skip -out $TMPDIR/${filename}_skip_mc -refvol 0 -plots -rmsrel -rmsabs -report
...
inplane=$(jq -e 'has("ParallelReductionFactorInPlane")' $DATAPATH/${filename}.json )
...
printf '%s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n %s\n' 'Task' 'Time' 'Vox' 'Slices' 'TR (s)' 'vols' 'TE (ms)' 'Acc' 'maxFD (mm)' 'meanFD (mm)' 'maxAbsMot (mm)' 'FD>0.2' 'vox-tSNR' 'sl-tSNR' 'mean BOLD' 'std BOLD' | paste -sd ',' > $OUTPATH/${filename}_qcvals.csv
