...
if [[ $filename = *echo-* ]] ; then
	...
	TMPDIR=$OUTPATH/tmp_echo-$echo_num
else
	TMPDIR=$OUTPATH/tmp
fi
...
num_vols=$(fslinfo $TMPDIR/${filename}_skip_mc | awk '{print $2}' | awk 'FNR == 5 {print}')
num_slices=$(fslinfo $TMPDIR/${filename}_skip_mc | awk '{print $2}' | awk 'FNR == 4 {print}')
tmpname=$(echo $RANDOM | md5sum | head -c 10)
fslsplit $TMPDIR/${filename}_skip_mc $tmpname
...
# Cleanup
if [[ $filename = *echo-* ]] ; then
	if [ ! -d $OUTPATH/echos ]; then
		mkdir -p $OUTPATH/echos
	fi
	mv $OUTPATH/${filename}_qcreport.pdf $OUTPATH/echos/${filename}_qcreport.pdf
fi
#rm -r $TMPDIR
rm $OUTPATH/${filename}*.png
