...
#SBATCH --output=/projects/b1134/processed/boldqc/logs/boldqc_ocrun_%a_%A.out
#SBATCH --error=/projects/b1134/processed/boldqc/logs/boldqc_ocrun_%a_%A.err
#SBATCH --job-name="boldqc_ocrun"       	# Name of job

# Braga Lab BOLD QC OC Run Pipeline
# Created by M. Lakshman on July 2021

# Usage:
# boldqc_ocrun_210713.sh file skip_trs
# e.g. sh /projects/b1134/tools/boldqc/boldqc_ocrun_210713.sh
...
for te_file in `ls $PROCESSEDDIR/*/sub-*/ses-*/*/*.csv`; do
    # read each lines
...
# Extract certain values from echo 1 csv

for echo1 in `ls $PROCESSEDDIR/*/sub-*/ses-*/*/*echo-1*.csv`; do
    # read each lines
    IFS=$',' read -d '' -r -a lines < $echo1

cat $echo1 | awk 'NR==2 {print $9}'
cat $echo1 | awk 'NR==2 {print $10}'
cat $echo1 | awk 'NR==2 {print $11}'
cat $echo1 | awk 'NR==2 {print $12}'
